...
stunting <- stunting11%>%
  select(l.wasting, c.age, c.sex, s.child, deliv, interval, bord, m.age, f.age, age.marri, 
...
str(stunting_22)
write.csv(stunting_22, "wasting1.csv")
summary(stunting_22)

stunting_22<-read.csv("wasting1.csv")
...
formula_stu <- as.formula(paste("l.wasting~", RHS1))
...
write.csv(dat, "Wasting_estimate.csv")


## Load dataset
scenario = 1
dfName = c('wasting1')[scenario]
yName = c('l.wasting')[scenario]
