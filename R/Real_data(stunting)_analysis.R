...
data<-read.csv("stunting1.csv", header = T)
str(data)
...
t_tau_2 = t_tau
t_tau_2 = t_tau + (log(0.5238232)*2-mean(t_tau)) #child size at birth
...
##Export result##
fname= paste('realdata_stunting_nonpara_result', '_outcome.Rdata', sep = '')
...
load(file = paste('realdata_stunting_nonpara_result', '_outcome.Rdata', sep=''))
attach(object)
...
##Export result##
fname= paste('realdata_stunting_para_result', '_outcome.Rdata', sep = '')
object= list(n_samp=n_samp,
             ATE_bias_null=ATE_bias_null, ATE_bias_BART0=ATE_bias_BART0,
             ATE_bias_BART1=ATE_bias_BART1, ATE_bias_CF=ATE_bias_CF,
             ATE_bias_CB0=ATE_bias_CB0, #ATE_bias_CB1=ATE_bias_CB1, 
...
load(file = paste('realdata_stunting_para_result', '_outcome.Rdata', sep=''))
attach(object)
ATE<-data.frame(ATE_bias_BART0, ATE_bias_BART1, ATE_bias_CF, ATE_bias_CB0)
CATE<-data.frame(CATE_RMSE_BART0, CATE_RMSE_BART1, CATE_RMSE_CF, CATE_RMSE_CB0, CATE_RMSE_CT, CATE_RMSE_CTOT)

(length(which(y_fact==1))/length(y_fact))*100
(length(which(data$c.size==0))/length(data$c.size))*100


PS1<-data.frame(x=PS[which(df[,zName]==0)])
PS2<-data.frame(x=PS[which(df[,zName]==1)])
length(PS2$x);length(PS1$x)
hist(PS2$x)
hist(PS1$x)

length(PS[df[,zName]==1])
length(PS[df[,zName]==0])
PS11<-data.frame(x=PS[df[,zName]==0])
PS22<-data.frame(x=PS[df[,zName]==1])
...
prs_df <- data.frame(pr_score = predict(log_fit_Z, type = "response"),
                     c.size = log_fit_Z$model$c.size)

labs <- paste("Child_size:", c("Treatment", "Control"))
prs_df %>%
  mutate(c.size = ifelse(c.size == 1, labs[1], labs[2])) %>%
...
prs_df$var<-ifelse(c.size==1, "Treatment", "Control")
ggplot(prs_df,aes(pr_score, group = var, color = factor(var), fill = factor(var), alpha = 0.2))+ 
  geom_histogram(breaks=seq(0.6,1,by=0.01), 
                 col="black",
                 alpha=0.2)+
  scale_color_manual(values=c("blue", "maroon"), aesthetics = c("colour", "fill"))+
  labs(title = "Propensity scores", x="Propensity", y="Density")+
...
library(dplyr)
PS1<-pr_score[c.size==1]
PS2<-pr_score[c.size==0]
